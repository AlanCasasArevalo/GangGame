...
class ModelUnitTest{
    ...
    fun dealParsingTest(){
        ...
        assertEquals(deal.title, "Toki Tori" )
        assertEquals(deal.metacriticScore, 80)
        assertEquals(deal.steamRating, 88 )
        assertEquals(deal.salePrice, 0.49F )
        assertEquals(deal.normalPrice, 4.99F )
        assertEquals(deal.thumb, "http://cdn.akamai.steamstatic.com/steam/apps/38700/capsule_sm_120.jpg?t=1488471030" )
    }
    ...
    fun topGameParsingTest(){
        ...
        assertEquals(topGame.title, "Counter-Strike")
        assertEquals(topGame.owners, 13364200)
        assertEquals(topGame.price, 999F)
        assertEquals(topGame.publisher, "Valve")
        assertEquals(topGame.thumb, "http://cdn.akamai.steamstatic.com/steam/apps/10/capsule_184x69.jpg")
